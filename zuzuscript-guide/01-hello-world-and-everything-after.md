...
3. **Pragmatic, not precious**<br>
   The language is designed for getting work done. It favors practical
...
## 1.2 Installing and running the Perl implementation
...
- `-d` or `-dN`  
  Enable runtime debug output (`-d` defaults to level 1).
...
Sometimes you just want to run one tiny idea quickly:
...
3. **`__main__` argument check**<br>
   ...
   "no argument".
4. **Module search path**<br>
   Create a tiny module in a custom directory and load it using `-I`.
5. **Mini status report**<br>
...
In Chapter 2, we will go deeper into values and types: how data is
represented, compared, combined, and transformed.
